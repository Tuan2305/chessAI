...
# Chịu trách nhiệm xử lý đầu vào của người dùng và hiển thị đối tượng Gamestate hiện tại
...
'''
    ...
    SET_BLACK_AS_BOT NÊN = FALSE

'''

# Đặt là True nếu chơi với quân trắng, False nếu chơi với quân đen
SET_WHITE_AS_BOT = False
SET_BLACK_AS_BOT = True
...
def loadImages():
    ...
    for piece in pieces:
        ...
        # p.transform.smoothscale chậm hơn một chút so với p.transform.scale, sử dụng để giảm độ nhòe pixel và tăng chất lượng hiển thị khi phóng to hình ảnh
...
def pawnPromotionPopup(screen, gs):
    ...
    text = font.render("Chọn quân để phong cấp:", True, p.Color("black"))

    # Tạo các nút cho lựa chọn phong cấp với hình ảnh
            ...
            if e.type == p.QUIT:
                ...
                for i, button in enumerate(buttons):
                    if button.collidepoint(mouse_pos):
                        if i == 0:
                            return "Q"  # Trả về quân được chọn
                        elif i == 1:
                            return "R"
                        elif i == 2:
                            return "B"
